...
<div class="row">
    <div class="col-md-3">
      ...
      <select name="names" class="input form-control">
        @foreach($client as $clients)
        <option value="{{$clients->names}}">{{$clients->names}}</option>
        @endforeach
      </select>
    </div>
    <div class="col-md-3">
      <label class="label">Rego: </label>
      <select name="rego" class="input form-control">
        @foreach($item as $items)
        <option value="{{$items->rego}}">{{$items->rego}}</option>
        @endforeach
      </select>
    </div>
</div>
